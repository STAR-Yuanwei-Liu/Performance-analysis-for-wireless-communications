...
for M_index = [1:1:length(Size)]
    ...
    Data(4,M_index) = C;
    C_zf=log2(1+SNR1*a1*(1-rho))+log2(1+SNR2*a2*(1-rho));
    %C_mrc=log2(1+SNR1*a1/(SNR2*a2*rho+1))+log2(1+SNR2*a2/(SNR1*a1*rho+1));
    Data(5,M_index) = C_zf;
    %Data(6,M_index) = C_mrc;
end
figure
...
semilogx(Size,Data(5,:),'--*g');hold on;
%semilogx(Size,Data(6,:),'--*b');hold on;